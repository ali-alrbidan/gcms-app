...
export default function SlaRulesPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(
        err instanceof ApiError ? err.message : t("slaRules.couldNotLoad"),
      );
    } finally {
      setLoading(false);
    }
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setFormError(
        err instanceof ApiError ? err.message : t("slaRules.couldNotSave"),
      );
    } finally {
      setSaving(false);
    }
    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">{t("slaRules.subtitle")}</p>
        </div>
        <button onClick={openCreate} className={primaryButtonClass}>
          {t("slaRules.newItem")}
        </button>
      ...
      <div className="mt-6 overflow-hidden rounded-lg border border-line bg-surface">
        {loading ? (
          ...
          <p className="p-6 text-sm text-muted">{t("slaRules.noItems")}</p>
        ) : (
          <table className="w-full text-left text-sm">
            <thead className="border-b border-line bg-paper text-xs uppercase tracking-wide text-muted">
              <tr>
                <th className="px-4 py-3 font-medium">
                  {t("slaRules.department")}
                </th>
                <th className="px-4 py-3 font-medium">
                  {t("slaRules.category")}
                </th>
              ...
              {rules.map((rule) => (
                ...
                >
                  <td className="px-4 py-3 text-ink">
                    {rule.department?.name ?? t("common.all")}
                  </td>
                  <td className="px-4 py-3 text-ink">
                    {rule.category?.name ?? t("common.all")}
                  </td>
                  <td className="px-4 py-3 text-ink">{rule.priority?.name}</td>
      ...
      {showForm && (
        <Modal
          title={editing ? t("slaRules.editTitle") : t("slaRules.createTitle")}
          onClose={() => setShowForm(false)}
        >
          <form onSubmit={onSubmit}>
            <FormField
              label={`${t("slaRules.department")} (${t("common.optional")})`}
            >
              <select
                className={inputClass}
                value={form.department_id}
              ...
              >
                <option value="">{t("slaRules.allDepartments")}</option>
                {departments.map((d) => (
                  <option key={d.id} value={d.id}>
                    {d.name}
                  </option>
            ...
            <FormField
              label={`${t("slaRules.category")} (${t("common.optional")})`}
            >
              <select
                className={inputClass}
                value={form.category_id}
              ...
              >
                <option value="">{t("slaRules.allCategories")}</option>
                {categories.map((c) => (
                  <option key={c.id} value={c.id}>
                    {c.name}
                  </option>
            ...
            <FormField label={t("slaRules.priority")}>
              <select
                required
                className={inputClass}
                value={form.priority_id}
            ...
            <FormField label={t("slaRules.responseHours")}>
              <input
                type="number"
                required
                min={1}
            ...
            <FormField label={t("slaRules.resolutionHours")}>
              <input
                type="number"
                required
                min={1}
                ...
                value={form.resolution_time_hours}
                onChange={(e) =>
                  setForm({ ...form, resolution_time_hours: e.target.value })
                }
              />
            </FormField>
            <label className="mb-4 flex items-center gap-2 text-sm text-ink">
