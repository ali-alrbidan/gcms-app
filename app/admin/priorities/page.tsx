...
export default function PrioritiesPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setFormError(
        err instanceof ApiError ? err.message : t("priorities.couldNotSave"),
      );
    } finally {
      setSaving(false);
    }
  ...
  async function onDelete(p: Priority) {
    const confirmMsg = tFn<[string]>("priorities.confirmDelete")(p.name);
    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">{t("priorities.subtitle")}</p>
        </div>
        <button onClick={openCreate} className={primaryButtonClass}>
          {t("priorities.newItem")}
        </button>
      </div>
      ...
      <div className="mt-6 overflow-hidden rounded-lg border border-line bg-surface">
        {loading ? (
          ...
          <p className="p-6 text-sm text-muted">{t("priorities.noItems")}</p>
        ) : (
          <table className="w-full text-left text-sm">
            <thead className="border-b border-line bg-paper text-xs uppercase tracking-wide text-muted">
              <tr>
      ...
      {showForm && (
        <Modal
          title={
            editing ? t("priorities.editTitle") : t("priorities.createTitle")
          }
          onClose={() => setShowForm(false)}
        >
          <form onSubmit={onSubmit}>
            <FormField label={t("priorities.name")}>
              <input
                required
                className={inputClass}
                value={form.name}
            ...
            <FormField label={t("priorities.code")}>
              <input
                required
                className={inputClass}
                value={form.code}
            ...
            <FormField label={t("priorities.level")}>
              <input
                type="number"
                required
                min={1}
            ...
            <FormField label={t("priorities.color")}>
              <input
                type="color"
                className="h-10 w-16 rounded-md border border-line"
                value={form.color}
            ...
            <FormField label={t("priorities.description")}>
              <textarea
                className={inputClass}
                rows={3}
                value={form.description}
