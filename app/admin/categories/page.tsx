...
export default function CategoriesPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(
        err instanceof ApiError ? err.message : t("categories.couldNotLoad"),
      );
    } finally {
      setLoading(false);
    }
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setFormError(
        err instanceof ApiError ? err.message : t("categories.couldNotSave"),
      );
    } finally {
      setSaving(false);
    }
  ...
  async function onDelete(cat: Category) {
    const confirmMsg = tFn<[string]>("categories.confirmDelete")(cat.name);
    ...
    <div>
      ...
      <div className="mt-6 overflow-hidden rounded-lg border border-line bg-surface">
        {loading ? (
          ...
          <p className="p-6 text-sm text-muted">{t("categories.noItems")}</p>
        ) : (
          <table className="w-full text-left text-sm">
            <thead className="border-b border-line bg-paper text-xs uppercase tracking-wide text-muted">
              <tr>
      ...
      {showForm && (
        ...
        >
          <form onSubmit={onSubmit}>
            <FormField label={t("categories.department")}>
              <select
                required
                className={inputClass}
                value={form.department_id}
            ...
            <FormField label={t("categories.code")}>
              <input
                required
                className={inputClass}
                value={form.code}
            ...
            <FormField label={t("categories.description")}>
              <textarea
                className={inputClass}
                rows={3}
                value={form.description}
            ...
            <FormField label={t("categories.keywords")}>
              <input
                className={inputClass}
                value={form.keywords}
                onChange={(e) => setForm({ ...form, keywords: e.target.value })}
