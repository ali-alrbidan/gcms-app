...
export default function EmployeeDashboard() {
  const { user } = useAuth(); const { locale, t } = useLocale(); const statusLabel = useStatusLabel(); const [complaints, setComplaints] = useState<Complaint[]>([]); const [meta, setMeta] = useState<PaginationMeta>(); const [loading, setLoading] = useState(true); const [error, setError] = useState<string | null>(null); const [status, setStatus] = useState(""); const [page, setPage] = useState(1); const [perPage, setPerPage] = useState(10);
  const load = async () => { setLoading(true); setError(null); try { const result = await employeeComplaintsApi.list({ page, per_page: perPage, scope: "assigned_to_me", ...(status ? { status } : {}) }); setComplaints(result.complaints); setMeta(result.meta); } catch (err) { setError(err instanceof ApiError ? err.message : t("complaints.couldNotLoad")); } finally { setLoading(false); } };
  ...
  const columns = useMemo<DataTableColumn<Complaint>[]>(() => [
    ...
    { id: "sla", header: "SLA", cell: (c) => c.is_sla_breached ? <span className="rounded-full bg-brick/10 px-2 py-1 text-xs font-medium text-brick">SLA</span> : <span className="text-xs text-muted">{c.sla_due_at ? new Intl.DateTimeFormat(locale === "ar" ? "ar" : "en-US", { dateStyle: "medium" }).format(new Date(c.sla_due_at)) : "—"}</span>, hideOnMobile: true },
